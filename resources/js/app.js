...
const COLOR_A = 0x4da6ff; // Bleu clair glacé
const COLOR_B = 0x004aad; // Bleu nuit
const COLOR_C = 0xa020f0; // Violet mystique
...
function createPlanet() {
  ...
  const material = new THREE.ShaderMaterial({
    uniforms: {
      ...
      colorC: { value: new THREE.Color(COLOR_C) },
      opacity: { value: 1.0 }
    },
    vertexShader: `
...
function createParticles() {
  ...
  const color1 = new THREE.Color(COLOR_A);
  const color2 = new THREE.Color(COLOR_B);
  const color3 = new THREE.Color(COLOR_C);
...
function createOrbitingPlanets() {
  const planetColors = [COLOR_A, COLOR_B, COLOR_C];
